...
void corp_loadBookingsFromCSV() {
    ...
    fclose(file);
    printf("\033[1;32m%d bookings loaded from corp.csv.\033[0m\n", lineCount);
}
...
void corp_main() {
    while (1) {
        ...
        // Menu options
        printf("\n");
        corp_displayCenteredText("\x31\xE2\x83\xA3  Book an Event", width, YELLOW BOLD);   // 1️⃣
        corp_displayCenteredText("\x32\xE2\x83\xA3  View All Bookings", width, BLUE);     // 2️⃣
        corp_displayCenteredText("\x33\xE2\x83\xA3  Exit", width, RED);                   // 3️⃣
        printf("\n");
        ...
        switch (choice) {
            ...
            case 3:
                system("clear");
                corp_saveBookingsToCSV(); // Save bookings before exiting
...
void corp_viewBookings() {
    ...
    corp_displayCenteredText("🎟️ All Bookings 🎟️", terminalWidth, MAGENTA BOLD); // 🎟️ All Bookings
