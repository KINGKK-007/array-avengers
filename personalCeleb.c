...
void personal_main()
{
    ...
    {
        int choice;
        int width = personal_getTerminalWidth(); // Get terminal width for dynamic adjustments
...
void personal_cat_display()
{
    ...
    // Display header
    printf("\n");
    personal_displayCenteredText("\xF0\x9F\x8E\x89 Event Categories", width, MAGENTA BOLD); // 🎉 Event Categories
    printf("\n");
...
int personal_getTerminalWidth()
{
    ...
    if (!fp)
        return 80; // Default width if the personalmand fails
    int width;
    fscanf(fp, "%d", &width);
    pclose(fp);
    return width;
...
void personal_viewBookings()
{
    ...
    int terminalWidth = personal_getTerminalWidth(); // Get terminal width
    ...
    for (int i = 0; i < terminalWidth; i++)
        printf("=");
    printf("\n");
    personal_displayCenteredText("🎟️ Your Bookings 🎟️", terminalWidth, MAGENTA BOLD);
    for (int i = 0; i < terminalWidth; i++)
        printf("=");
    printf("\n");
    ...
    if (file == NULL)
    {
        personal_displayCenteredText("❌ No bookings file found. ❌", terminalWidth, RED);
        printf("\n");
        return;
    }
    ...
    if (!foundBooking)
    {
        personal_displayCenteredText("❌ No bookings found for your mobile number. ❌", terminalWidth, RED);
    }
